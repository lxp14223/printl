...
class ImageDetector:
    ...
    def detect_defects_with_cc(self, template, comparison_image, min_area=10, margin=50):
        """
        按连通域分块检测缺陷
        ...
        for i in range(0, num_labels):
            try:
                ...
                # 打印进度
                print(f"连通域 {i}/{num_labels - 1} 检测完成，发现 {len(roi_regions)} 个缺陷")
        ...
        # ========== 第6步：误检过滤 ==========
        final_regions = self.filter_false_positives(final_regions, template, comparison_image)
    ...
    def detect_defects(self, template, matched_region):
        ...
        # 二次对齐
        matched_gray, homography = align_images_with_distortion(template_gray, matched_gray)
        ...
        if self.dilate_iter > 0:
            matched_binary = cv2.dilate(matched_binary, kernel, iterations=self.dilate_iter)
        ...
        if self.erode_iter > 0:
            matched_binary = cv2.erode(matched_binary, kernel, iterations=self.erode_iter)
        ...
        # ==================== 步骤8：缺陷掩膜生成 ====================
        overlay = np.zeros((template_fg.shape[0], template_fg.shape[1], 3), dtype=np.uint8)
    ...
    def _extract_defect_regions(self, diff):
        ...
        # 创建标注图片（彩色，用于显示）
        diff_color = cv2.cvtColor(diff, cv2.COLOR_GRAY2BGR) if len(diff.shape) == 2 else diff.copy()
        ...
        for contour in diff_contours:
            ...
            if area >= self.min_area:
                ...
                if fill_ratio >= self.fill_ratio_threshold:
                    ...
                    cv2.putText(diff_color, 'Diff', (x, y - 5),
    ...
    def set_params(self, diff_tolerance=30, min_area=20, fill_ratio_threshold=0.1,
                  morph_kernel=3, erode_iter=1, dilate_iter=0, nms_iou_threshold=0.2,
                  nms_distance=30):
    ...
    def filter_false_positives(self, regions, template, matched):
        ...
        print(f"误检过滤: {len(regions)} -> {len(filtered)} (过滤 {len(regions) - len(filtered)} 个)")
        return filtered

    # ==================== 论文方法增强 ====================
    
    def compute_local_structure_response(self, image, reference_region=None, epsilon=1e-6):
        """
        计算局部结构响应函数（论文公式2）
    ...
    def compute_position_weight(self, template, edge_weight=2.0, texture_weight=1.5):
        ...
        # 计算局部标准差
        kernel_size = 9
        mean_local = cv2.boxFilter(gray.astype(np.float32), -1, (kernel_size, kernel_size))
        var_local = cv2.boxFilter((gray.astype(np.float32) - mean_local)**2, -1, (kernel_size, kernel_size))
    ...
    def compute_multi_feature_fusion(self, template, matched, 
                                     w_gray=1.0, w_edge=1.0, w_color=1.0):
        """
        三特征融合判别（论文公式4）
        ...
        if len(template.shape) > 2 and len(matched.shape) > 2:
            color_diff = np.max(np.abs(matched.astype(np.float32) - template.astype(np.float32)), axis=2)
            color_diff_norm = cv2.normalize(color_diff, None, 0, 1, cv2.NORM_MINMAX)
        else:
            color_diff_norm = gray_diff_norm
        
        # 融合
        fusion_map = w_gray * gray_diff_norm + w_edge * edge_diff_norm + w_color * color_diff_norm
        ...
        feature_dict = {
            ...
            'color_diff': color_diff_norm.astype(np.uint8) if isinstance(color_diff_norm, np.ndarray) else gray_diff.astype(np.uint8),
            'fusion': fusion_map
        }
