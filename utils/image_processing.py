...
def align_images_with_distortion(template, target, min_match_count=10):
    ...
    target_gray = cv2.cvtColor(target, cv2.COLOR_BGR2GRAY) if len(target.shape) > 2 else target
...
"""锐化图像 - 解决待检测图片模糊问题"""
def sharpen_image(image, method='usm', strength=1.0):
    """
    锐化图像，解决待检测图片模糊的问题
