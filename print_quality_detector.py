...
def remove_colors_from_image(image, colors_to_remove, tolerance=20, replace_color=None):
    ...
    white_mask_expanded = cv2.dilate(white_mask, kernel_dilate, iterations=2)

    mask = cv2.bitwise_and(mask, cv2.bitwise_not(white_mask_expanded))
    ...
    if len(light_pixels[0]) > 0:
        ...
        for y, x in zip(light_pixels[0], light_pixels[1]):
            mask[y, x] = 0

    print(f"去除颜色: 找到 {np.sum(mask > 0)} 个像素需要去除（已排除白色和浅色）")
    ...
    print("使用inpainting算法自动填充（半径=5，减少对边缘影响）")
...
class PrintQualityDetector(QMainWindow):
    ...
    def init_ui(self):
        ...
        params_layout.addWidget(apply_params_btn, 11, 0, 1, 3)
    ...
    def update_sharpen_strength_label(self, value):
        """更新锐化强度标签"""
        strength = value / 10.0
        self.sharpen_strength_value.setText(f"{strength:.1f}")
    ...
    def apply_params(self):
        ...
        use_dnn = self.dnn_align_checkbox.isChecked()
        if mode == 0:  # 全功能检测
