...
from MvImport.camera_worker import CameraCaptureDialog
...
def extract_main_colors(image, n_colors=12):
    ...
    _, labels, centers = cv2.kmeans(data, n_colors, None, criteria, 3, cv2.KMEANS_RANDOM_CENTERS)
...
def remove_colors_from_image(image, colors_to_remove, tolerance=30):
    """从图像中去除指定的颜色，使用附近颜色填充"""
...
class PrintQualityDetector(QMainWindow):
        ...
        if ext == '.pdf':
            ...
            try:
                ...
                QMessageBox.information(self, "成功",
                                        f"PDF 已加载!\n尺寸: {w}x{h}\n分辨率: 300 DPI\n\n请点击\"提取颜色\"去除不需要的颜色，或点击\"框选模板区域\"选择模板范围。")
    ...
    def extract_template_colors(self):
        ...
        try:
            ...
            colors = extract_main_colors(self.template, n_colors=12)
            QApplication.restoreOverrideCursor()

            if not colors:
                QMessageBox.warning(self, "提示", "无法提取颜色信息!")
                return
            ...
            if dialog.exec_() == QDialog.Accepted and dialog.selected_colors:
                ...
                if reply == QMessageBox.Yes:
                    QApplication.setOverrideCursor(Qt.WaitCursor)
                    self.template = remove_colors_from_image(self.template, dialog.selected_colors, tolerance=30)
                    QApplication.restoreOverrideCursor()
    ...
    def start_template_selection(self):
        """开始框选模板区域"""
        if self.pdf_full_image is None:
            QMessageBox.warning(self, "提示", "请先加载 PDF 文件!")
            return
    ...
    def on_template_roi_selected(self, x1, y1, x2, y2):
        ...
        h, w = self.pdf_full_image.shape[:2]
        x1 = max(0, min(x1, w - 1))
        y1 = max(0, min(y1, h - 1))
        x2 = max(0, min(x2, w - 1))
        y2 = max(0, min(y2, h - 1))
        ...
        cropped = self.pdf_full_image[y1:y2, x1:x2].copy()
    ...
    def load_images(self):
        ...
        try:
            ...
            if dialog.exec_() == QDialog.Accepted and dialog.captured_image is not None:
                ...
                temp_dir = "captured_images"
                os.makedirs(temp_dir, exist_ok=True)
                save_path = os.path.join(temp_dir, temp_filename)
                cv2.imwrite(save_path, image)

                self.image_list.append(save_path)
                self.image_list_widget.addItem(temp_filename)

                self.current_image = image.copy()
                self.current_image_path = save_path
                self.switch_display_mode("current")

                QMessageBox.information(self, "成功", f"已从相机采集图像: {temp_filename}")
            else:
                print("取消相机采集")
        finally:
            if dialog is not None:
                dialog.worker.stop_grabbing()
                dialog.worker.close_device()
                dialog.close()
                dialog.deleteLater()
    ...
    def detect_current(self):
        ...
        try:
            ...
            if 'image_defects' in result and result['image_defects'].get('significant_diff') is not None:
                ...
                self.vis_data['missing'] = result['image_defects'].get('missing')
                self.vis_data['extra'] = result['image_defects'].get('extra')
    ...
    def view_details(self):
        ...
        if 'template' in self.vis_data and 'matched_region' in self.vis_data:
            dialog = ZoomCompareDialog(
                ...
                self.vis_data.get('aligned_region'),
                self.vis_data.get('defect_mask'),
                self.vis_data.get('missing'),
                self.vis_data.get('extra'),
                self.vis_data.get('defect_regions'),
                self
            )
            dialog.exec_()
        else:
