...
class DetectionEngine:
    ...
    def detect_print_quality(self, template, scanned_image, color_sample_points=None):
        ...
        if self.sharpen_scanned:
            print(
                f"锐化待检测图片 (方法={self.sharpen_method}, 强度={self.sharpen_strength})..."
            )
            scanned_image = sharpen_image(
                scanned_image,
                method=self.sharpen_method,
                strength=self.sharpen_strength,
            )
        ...
        if self.align_images_flag:
            if self.use_dnn_alignment:
                ...
                aligned_image, homography = align_images_with_dnn(
                    template, scanned_image
                )
        ...
        if self.detect_text_errors:
            ...
            text_errors = self.text_detector.detect_text_errors(
                template_text, scanned_text
            )
        ...
        if self.detect_color_differences and color_sample_points:
            template_colors = self.color_detector.measure_color(
                template, color_sample_points
            )
            scanned_colors = self.color_detector.measure_color(
                comparison_image, color_sample_points
            )

            color_differences = self.color_detector.compare_colors(
                template_colors, scanned_colors
            )
